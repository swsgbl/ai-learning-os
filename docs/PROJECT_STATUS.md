...
| M2-05 答案 append-only event | 本分支 | pytest 119 passed（含真实 PG）：乱序 409、同内容幂等、同序号不同答案拒绝且原事件不变（API+事件流双断言）、拒绝后事件流原样 [1,2,3]、交卷后禁止追加 5 测；真实 PG 并发同序号 gather → 一成功一明确 ValueError（flush 撞 uq 转 409，无 500）1 测；grep 确认无 update/delete 事件路径；前端门禁绿 | 2026-08-31 |

## 待办任务（按 backlog 顺序）

- [ ] M2-06 自动保存与断线恢复；M2-07 幂等提交与超时提交
