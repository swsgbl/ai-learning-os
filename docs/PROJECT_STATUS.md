...
| 36 | 语音答案规范化边界：normalizer 只规范化不判定（「最终提交由服务端判定」），规范化语义与判分器同源（共用 normalize_type；true_false 归一与 grading._normalize_tf 同规则）；服务端从 transcript 重新解析+规范化，不信任客户端 normalized_answer/confidence（04 文档「不信任客户端分数」）；event_id 为客户端幂等键（voice_answer_events 主键+并发重放捕获回读），重放返回既有结果不改状态不重复落库；无效答案（选项不存在/序号超界/题型不支持）→澄清且 accepted=false 审计留痕不含答案；essay 拒绝语音作答（rubric 管线走书面提交） | M4-05 验收「语音答案只生成规范化答案事件，最终提交由服务端判定」+ 04 文档 /answers 契约 | 2026-09-01 |
