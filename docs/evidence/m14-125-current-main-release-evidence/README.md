# M14-125：current-main 发布证据刷新（PR #211 后；ci-main + release-check 真实重跑 + provider-smoke/long-soak 契约内只读复用 + cockpit 零 blocker 聚合）

- 切片：分支 `ops/m14-125-current-main-release-evidence`（独立 worktree
  `m14-125-current-main-release-evidence`，基于 main
...
- 动因与定性：M14-123 的 final-current 证据绑定执行基点 d7072fd，其后
  supervisor 合并轮引入两轮 docs-only 变更——PR #210（M14-123 证据
  入库，4 文件 +197/−0）→ `684865d`，PR #211（M14-124 预审批构建
  证据回填，4 文件 docs）→ `e14be369`。代码绑定门相对合并头即呈
  docs-only 漂移（M14-124 README 已如实登记该漂移并明确「接受或刷新
  由 supervisor 判断」）——本切片即 supervisor 指令的真实重执行刷新：
  在**当前 HEAD `e14be369`** 上重执行 ci-main 与 release-check 两门，
  证据基点即本切片执行树基点。
- 目标：基于 M14-91/M14-116/M14-123 既有契约刷新 current main 的发布
  证据链。ci-main 与 release-check 两门**本轮真实重执行**（远端 CI 事实
  核验 + 干净 e14be369 执行树上的隔离 full 跑）；**provider-smoke 只读
  复用 M14-117 生产切换后的真实三步冒烟聚合**（与 M14-122/M14-123
  同一源、同一 SHA256，源哈希与语义仍有效的契约内复用——生产仍运行
  `m14-117-production` 栈，无新切换，无新冒烟窗口指令）；long-soak 沿
  M14-107/M14-116/M14-122/M14-123 同款只读复用（同哈希
  `d939c652…`，原窗口时间边界如实呈现）。绝不手改 gate JSON、绝不
  合成 pass、绝不搬运与源哈希不符的旧 JSON 冒充 current、绝不重跑
  provider/生产、绝不合成为切换后栈伪造的新 24h soak。
...
- 零生产触碰（本切片全程）：零容器启停/重建、零计划任务操作、零
  ...
  127.0.0.1:7892）+ worktree 从零环境的包安装（uv/npm，日志归档
  §5）。**provider-smoke 与 long-soak 均不重跑**——两者为只读校验 +
  哈希锁定复用（§2.3/§2.4），源文件零改动。
...
- 本 README 为唯一入库证据文件；canonical 原始证据（gitignored）位于
  worktree `.verify/artifacts/m14-125-current-main-release-evidence/`
  （8 份核心工件 + 执行日志/脚本，SHA256SUMS 索引 24 文件，自不含
  自哈希；§5）。
...
| 事件 | commit | 说明 |
|------|--------|------|
| PR #209 merge（M14-122 证据入库 + supervisor 合并轮） | `d7072fdcd97b233a58386693e27bcd8288975cca` | M14-123 执行基点（final-current） |
| PR #210 merge（M14-123 证据入库） | `684865d1212ca565182fd04edcb3f6a21c9ef88b` | docs-only（4 文件 +197/−0：evidence README + 三台账） |
| PR #211 merge（M14-124 预审批构建证据回填）→ **当前 main** | `e14be369a4a80267dcaafd41f7475cd44e2166ed` | docs-only（4 文件：evidence README + 三台账）；本切片精确基点 |
| main push CI run 35992664886 创建（push 事件） | e14be369 | 2026-09-24T11:22:45Z（UTC） |

- `d7072fd..e14be369` 两轮合并均为 docs-only（M14-124 已独立核验
  `2619ea7..684865d` 17 文件全部在镜像构建输入面外；PR #211 同样仅
  docs/evidence README + 三台账），**services/api 测试面零变更**——
  本切片 release-check 的 pytest 计数与 M14-123 恰一致（4555/33，
  +0 对账，§2.2），这本身即 docs-only 区间的精确实证；重执行按
  supervisor 指令真实发生（非因测试变更），证据绑定刷新至当前 HEAD。
- M14-123 canonical 保留为其时点历史记录不改写（其 worktree 只读取
  证——本切片 long-soak 复用即逐字节读其 canonical §2.4，零修改）；
  本切片不搬运其 ci-main/release-check 产物——run 35972347264 与
  d7072fd 上的 release-check 结果均不复用。M14-83（@ 5829ad9 生产
  只读门三门）、M14-85（@ f47a1e4 backup-restore）、M14-87（@
  audit-chain-anchor 门 + 伴生锚）canonical 按 M14-91 §5 登记路径
  **原样 staging**（§2.5）。
- **provider-smoke 复用源不变**：生产自 M14-117 切换后始终运行
  `m14-117-production` 栈（M14-124 只读复核再次确认 7 容器 healthy
  且仍运行该栈），本切片零切换零 provider 接触，M14-117 切换后聚合
  （`029ee84f…`，564 bytes，generated_at 2026-09-23T23:29:35Z）仍是
  与当前生产状态对应的最新真实 pass 证据——按「源哈希与语义仍有效才
  复用」口径，与 M14-122/M14-123 同源同哈希复用（§2.3）。
- long-soak 复用口径与 M14-107/M14-116/M14-122/M14-123 完全相同：
...
```bash
gh api "repos/swsgbl/ai-learning-os/actions/runs?head_sha=e14be369a4a80267dcaafd41f7475cd44e2166ed"
gh api "repos/swsgbl/ai-learning-os/actions/runs/35992664886/jobs"
```
...
- 命中断言（断言脚本复核归档 raw 响应而非仅信任务简报；任一断言
  ...
  event=push、head_branch=main 恰 1 条——run **35992664886**
  （run_number **543**），head_sha `e14be369…`，status=completed，
  **conclusion=success**（created 2026-09-24T11:22:45Z / updated
  2026-09-24T11:26:58Z，
  https://github.com/swsgbl/ai-learning-os/actions/runs/35992664886）。
- jobs 断言：`total_count=5` 且全部 conclusion=success——API
  ...
  Web（test/typecheck/lint/build）；每 job head_sha == e14be369、job 名
  集合精确匹配且无重复（与 M14-116/M14-122/M14-123 同一五 job 契约）。
- 按 `_eval_ci_main` 契约（`services/api/app/ops/release_readiness.py`）
  **程序化派生** canonical `evidence/ci-main.json`（1203 bytes、
  SHA256 `10849244…`）：断言驱动脚本从 raw 事实拼装 + 溯源 source 串；
  未复用 M14-123 的 run 35972347264 或其 raw 文件。
- raw API 响应原样归档：`raw/gh-runs-e14be369.json`、
  `raw/gh-jobs-35992664886.json`（哈希见 §5；两份 stderr 捕获为空
  文件）。

### 2.2 release-check（隔离本地 full 重跑，干净 e14be369 执行树）
...
M14-92/M14-94/M14-97/M14-105/M14-107/M14-116/M14-122/M14-123 相同的
已知提示，如实记录；该脚本产物非门禁依赖，本轮 10 门全绿证实无
影响。）

**执行树前置核验**（declared-head 诚实性前提）：运行前
`git rev-parse HEAD` == `e14be369…` 且 `git status --porcelain` 为空
（tracked-clean，仅 gitignored `.verify/`/`artifacts/` 增量，执行日志
归档留痕）——release-check 的实际执行树即精确干净 e14be369，后续
cockpit `--gate-declared-head release-check=e14be369` 声明成立。
...
`artifacts/m14-125-isolated/`）：

```bash
cd services/api
"<worktree>/.venv/Scripts/python.exe" -m app.ops.cli release-check-isolated \
  --workdir "<worktree>/artifacts/m14-125-isolated" --json
# exit 0
```

- 结果：**all_green=true，10/10 pass**（generated_at
  2026-09-24T11:48:13.595559+00:00，execution_scope=full）——api-lint
  （ruff All checks passed）/ web-lint / web-typecheck / web-build /
  **api-test（pytest 4555 passed, 33 skipped, 1 warning in 277.38s；
  与 M14-123 @ d7072fd 的 4555 恰 +0——d7072fd..e14be369 区间两轮
  合并均 docs-only、services/api 测试面零变更的精确对账，实测吻合、
...
- 复用源：**M14-117 生产切换轮固化的切换后真实聚合产物**
  ...
  `m14-117-production` 新栈上）。**与 M14-122/M14-123 所 stage 的是
  同一源文件同一哈希**——生产栈自 M14-117 切换后未再变更（M14-124
  只读复核佐证），该源的语义有效性（对应当前生产状态）未被任何后续
  切换事件打破，按既有契约（「源哈希与语义仍有效才复用」）本切片
  复用成立。
...
- 该 pass 证据的执行与 supervisor 复核属 M14-117 切换轮（search 冒烟
  ...
  §7 / M14-116 §6.3 / M14-122 §6.3 / M14-123 §6.3 口径保持——
  **原始时间边界显式：2026-09-23T23:29:35Z，距本切片聚合时点
  （2026-09-24T11:50:34Z）约 12 小时，窗口外状态不承诺**。

### 2.4 long-soak（不重跑、不触碰生产/计划任务；只读校验 + 逐字节复制）

- 复用口径与 M14-107 §2.3 / M14-116 §2.4 / M14-122 §2.4 / M14-123
  §2.4 完全一致：canonical 817 bytes、SHA256
  `d939c6528b9fe739270fe6ea706837ffa9dc13115665dc90740e6db53a1056ec`
  （与 M14-123 §5 登记值一致，大小写不敏感）——即 M14-106 运维轮
  supervisor 复核的真实 24h 稳定窗口审计（M14-107 首次 stage、
  M14-116/M14-122/M14-123 复用的同一证据；本切片直接读 M14-123
  canonical 复核后逐字节复制）。
...
在 tracked docs 编辑**之前**、HEAD 仍为干净 `e14be369` 时运行。canonical
源路径逐一取自 M14-91 README §5 / staged-inventory 登记与 M14-123
§2.5 同一路径（不猜测）；staging 前先对 6 个既有生产状态源文件做
sha256 复核——与 M14-91/M14-116/M14-122/M14-123 登记值逐一
**MATCH（6/6）**：audit-chain-anchor `a8c54c5e…` / audit-anchor
companion `d2bfd877…` / backup-restore `ed0fc5b4…` / preflight
`b3a2be66…` / legacy `83cd62bd…` / draft `e90ae04c…`（执行记录
...
"<worktree>/.venv/Scripts/python.exe" -m app.ops.cli evidence-cockpit \
  ...
  --gate-declared-head release-check=e14be369a4a80267dcaafd41f7475cd44e2166ed \
  --current-head e14be369a4a80267dcaafd41f7475cd44e2166ed \
  --staging-dir "<worktree>/artifacts/m14-125-cockpit-staging" \
  --json --output "<worktree>/artifacts/m14-125-cockpit-report.json"
# exit 0（cockpit_ready=true）
```

- **current-head 与 release-check head 显式声明
  `e14be369a4a80267dcaafd41f7475cd44e2166ed`**；`--gate-declared-head
  release-check` 的合法性由 §2.2 前置核验支撑（实际执行树即干净
  e14be369；ci-main 则由文件内嵌 merge_commit==current HEAD 自动
  current）。
- 结果（报告归档 canonical `cockpit-report.json`，generated_at
  2026-09-24T11:50:34.018362+00:00）：**evaluator pass=9 / pending=0
...
- **code-bound 两门刷新生效**：ci-main stale=**current**（内嵌
  merge_commit=e14be369 == current HEAD）、release-check stale=
  **current**（flag 声明 e14be369）。
- **provider-smoke 以 M14-117 切换后聚合 staged 即 pass**（经
  ...
  不阻断」摘要显式区分。
- **production-preflight 门的取源说明（诚实记录，与 M14-122/M14-123
  同）**：M14-117 切换后 preflight（5/5 pass，2026-09-23T23:14:45Z）
  是更新的生产状态事实，但其 JSON **不含 evaluator 契约要求的
  `gate` 自声明字段**，stage 它将触发 MalformedEvidence fail-closed
  ——本切片绝不手改补字段，故该门继续 stage M14-83 注册 canonical
  （`b3a2be66…`，6/6 哈希复核通过），M14-117 切换后 preflight 作为
  「staged 各门语义在切换后仍成立」的支撑事实在 README 引用（其
  5 检查在切换后复核了 audit-chain valid / legacy 三类计数零 /
  draft 两类计数零 / 锚定 up-to-date / alembic current==head，恰为
  staged 五门所覆盖的生产状态面）。
- **staged 10 文件（9 门 + anchor companion）与 source 逐字节一致**：
  ...
  值交叉一致、本切片 4 个新 stage 文件（ci-main `10849244…` 1203
  bytes / release-check `4153275b…` 2491 bytes / provider-smoke
...
cd services/api && "<worktree>/.venv/Scripts/python.exe" -m pytest -q \
  ...
  --basetemp "D:/AI Learning OS/.pytest-tmp/m14-125-current-main-release-evidence/focused"
# 407 passed, 1 warning in 6.59s
```

（Windows 已知形态：pytest `--basetemp` 不创建缺失父目录链，本切片
按 M14-116/M14-122/M14-123 教训**先 `mkdir -p` 预建父目录**，一次
通过。）
...
- canonical 完整性：`sha256sum -c SHA256SUMS` **24/24 OK**（本切片
  ...
  **22 项全过**（ci-main run_id/merge_commit/conclusion、raw runs
  push@main 唯一/run_number 543、raw jobs 5/5 精确集合、release-check
  all_green 10/10 + pytest 4555/33 + 对 M14-123 增量 +0 对账、
  ...
  IDENTICAL）。
- 秘密扫描：canonical 24 文件对 credential 赋值 / OpenAI 风格 key /
...
## 4. 与 M14-123 的差异（同一契约下的净变化）

| 维度 | M14-123 @ d7072fd | M14-125 @ e14be369 |
|------|-------------------|--------------------|
| 证据定性 | final-current（基点后被两轮 docs-only 合并超越） | **current-main（证据基点 = 切片执行基点 = 当前 HEAD e14be369）** |
| 漂移性质 | 2b1c2dd..d7072fd 含 M14-121 真实测试文件（+3） | **d7072fd..e14be369 两轮合并均 docs-only（零测试面变更，+0 对账）** |
| ci-main run | 35972347264（run_number 539） | **35992664886**（run_number 543，本轮新派生） |
| release-check pytest | 4555 passed / 33 skipped（281.16s） | **4555 passed / 33 skipped**（277.38s，+0 = docs-only 区间对账） |
| provider-smoke 复用源 | M14-117 切换后聚合（`029ee84f…`） | **同源同哈希复用（生产栈未变，语义有效性保持；M14-124 复核佐证）** |
...
路径：worktree `.verify/artifacts/m14-125-current-main-release-evidence/`

| 文件 | bytes | sha256 |
|------|-------|--------|
| `evidence/ci-main.json` | 1203 | `10849244d71d56c23579af62da87690b94fb8818b4ab1345ec4695b90fc0586b` |
| `evidence/release-check.json` | 2491 | `4153275b0ec9f1600f90fc636ee8051b29eda8f2b802bc1212b1f47340b0394a` |
...
| `raw/gh-runs-e14be369.json` | 12222 | `b610bea00ce9fdb6a7b0f418f165cb77175df811d283a80174d5128fc85d8950` |
| `raw/gh-jobs-35992664886.json` | 12334 | `e802d3de485e635e29a57440d0a8a439d213b2e33aac5c06098df4aea418c2cc` |
| `cockpit-report.json` | 19002 | `e79410bec55c7ee8390f3a89ef35a1c9f4a8bdda64032ea881ffc37e285f3957` |
| `staged-inventory.txt` | 2778 | `f95c4d86b56ff8c27173b51e5d6d58628de2ea2cfc99ac1f4b0d890bcfd79eea` |

（另 `logs/` 下执行日志与断言/派生/校验脚本 14 份（env-setup/
derive-ci-main + 派生脚本/prepare-reused-evidence + 校验脚本/
release-check-isolated 双流/cockpit 双流 + stdout JSON/verify-staging +
复核脚本/assert-contracts + 断言脚本）+ `raw/*.stderr.txt` 两份空捕获
（SHA256 `e3b0c442…` = 空文件标准值）+ `SHA256SUMS` 24 文件索引，
自不含自哈希；worktree 侧 `artifacts/m14-125-isolated/`、
`artifacts/m14-125-cockpit-staging/`、
`artifacts/m14-125-cockpit-report.json` 为生成现场，gitignored，
不入库。）
...
3. **provider-smoke pass 是 M14-117 切换轮的时点证据**
   ...
   约 12 小时）；易失性由后续刷新切片覆盖（M14-98 先例）。本切片
...
6. **生产当前运行 m14-117-production**（M14-117 于 2026-09-24 切换，
   M14-124 只读复核佐证 7 容器 healthy），本切片零调度面触碰、不构成
   任何部署；不授权任何部署。M14-124 登记的 `m14-124-production`
   两枚审批前镜像与回滚锚材料保持有效，等待 hash-bound human-only
   审批绑定。
7. **代码绑定门证据只对其执行时点的树成立**：ci-main/release-check
   现对 e14be369 current（= 本切片基点 = current main）；main 再前移
