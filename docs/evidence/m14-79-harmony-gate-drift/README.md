...
- **Gap B（agc_closure_manifest 零交叉校验，已采纳）**：三份门报告
  ...
  HAP）或混搭证据（不同构建的 HAP 哈希混用）在逐门状态全 pass 时仍
  manifest `ok`。这是纯新增检查，完全离线可实现，不翻转任何既有契约。
...
- `build_manifest`（L448 初始化 `chosen`，L477 记录每门选定报告，L495
  调用）：双侧哈希都在且不等 → 新增 blocker
  `evidence_hap_mismatch`（`gate=evidence_chain`，`detail.link` 指明断链
  位置）→ manifest `blocked`（exit 2）。单侧缺失不触发（缺失证据仍归
  `missing_report` / 逐门检查管辖，既有行为不变）。
...
4. `test_missing_records_never_trigger_chain_blocker`：报告无 HAP 记录时
   链检查静默（不新增 blocker），既有 missing-evidence 行为不变。
