...
1. CLI 注册与分发：两个子命令注册进主 help、必填参数缺失 exit 2、非法
   ...
   parser 注册区无 --yes 执行旗标；
2. 映射稳定性：PROVIDERS 三槽位（provider/脚本相对路径/step/gate_key）与
...
9. 零敏感信息落盘：单步证据顶层键恰为白名单（tool + 任务契约八字段，
   与 STEP_EVIDENCE_KEYS 交叉锁定）、聚合 providers 每项仅 executed/result；
...
def _aggregate_cli(
    search: Path, cloud_voice: Path, llm: Path, output: Path, *, as_json: bool = False
) -> int:
    return cli_module._run_provider_smoke_aggregate(
        SimpleNamespace(
            search=str(search),
            cloud_voice=str(cloud_voice),
            llm=str(llm),
            output=str(output),
            as_json=as_json,
        )
...
def _write_three_steps(
    ...
    outcomes = dict(zip(sorted(PROVIDERS), results, strict=True))
    return {
...
def test_provider_mapping_matches_rehearsal_and_readiness() -> None:
    ...
    assert sorted(PROVIDERS) == ["cloud-voice", "llm", "search"]
...
def test_aggregate_one_fail_exit_1_evidence_written(tmp_path, capsys) -> None:
    ...
    artifacts = _artifacts(tmp_path)
    # _write_three_steps 槽序为 sorted(PROVIDERS)：cloud-voice/llm/search
...
def test_aggregate_contract_is_minimal(tmp_path) -> None:
    ...
    executed/result；顶层不得透传单步 exit_code/时间/脚本细节。"""
    artifacts = _artifacts(tmp_path)
    ...
    for entry in evidence["providers"].values():
        assert set(entry) == {"executed", "result"}
...
def test_aggregate_evidence_passes_readiness_evaluator_directly(
    ...
    assert status == expected
    assert set(data) == set(SMOKE_PROVIDERS)
...
def test_written_aggregate_consumed_by_full_readiness(tmp_path, fake_runner) -> None:
    ...
    assert gate["data"] == {
        name: {"executed": True, "result": "pass"} for name in SMOKE_PROVIDERS
    }
