...
- restart 策略：所有长驻服务（postgres/redis/minio/api/livekit/web/searxng）恒
  `restart: unless-stopped` —— 经 docker compose config 渲染断言（local/
  hybrid/cloud/search/无 profile 五形态，不启动任何容器）；无 Docker CLI 时回退
  PyYAML 静态解析（CI 可跑），两者互补。M14-66: searxng 挂独立 search
  profile，只在 --profile search 形态渲染进服务集（渲染面），静态面全集含它。
...
LONG_RUNNING_SERVICES = ("postgres", "redis", "minio", "api", "livekit", "web", "searxng")
PIN_KEYS = (
    "AIOS_IMAGE_TAG",
    "AIOS_WEB_IMAGE_TAG",
    "AIOS_APP_ENV",
...
@pytest.mark.parametrize("profile,expected_services", [
    ...
    # M14-66: search profile 只追加 searxng（语音 livekit 不隐含——两独立开关）
    ("search", {"postgres", "redis", "minio", "api", "web", "searxng"}),
...
def test_static_yaml_every_long_running_service_has_unless_stopped() -> None:
    """无 Docker CLI 的静态回退：YAML 直解，七个长驻服务全部声明 restart: unless-stopped。"""
