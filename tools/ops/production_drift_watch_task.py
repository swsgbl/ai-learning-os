#!/usr/bin/env python
"""M14-129 production drift watch 独立周期计划任务 readiness 管理器。

管理隐藏周期计划任务（Task Scheduler），把 tools/ops/production_drift_watch.py
经静默 VBS wrapper（run_production_drift_watch_silent.vbs）挂到固定间隔触发。
**独立任务**：与 M14-14 AIOS-Monitoring-Pipeline 互不接入/互不改动（不同任务
名/URI/wrapper），两任务同机并存、各自独立管理。子命令：``plan``（只读
预检+计划）/ ``generate``（导出任务 XML 供审查——**UTF-16 with BOM 字节**，
与 XML 声明及 install 临时字节一致、外部解析器可直接加载；零调度器改动）/
``status``（只读状态）/ ``install`` / ``uninstall``。

设计纪律（与 tools/ops/monitoring_pipeline_task.py 同源的 M14-06/M14-14 已验证
模式；windows_startup_task.py M14-06 R2–R4 实证先例）：
...
- **install/uninstall 各自需精确确认短语**（``--confirm
  "EXECUTE PRODUCTION DRIFT WATCH SCHEDULER CHANGE"`` 一字不差），缺一/近似
  即拒绝且零 schtasks 调用；**本开发回合零安装/零卸载/零注册**——实际注册
  是 supervisor-only（获准窗口 + 提升令牌），本工具只交付 readiness。
- 绝不覆盖同名任务：install 前必先只读 query + 二次全量列表复核，已存在
  ...
  查询失败绝不 force、绝不删除。
- 固定任务身份 + 保守重复间隔：``AIOS-Production-Drift-Watch`` /
  ``urn:aios:m14-129:production-drift-watch``；TimeTrigger 重复间隔 **PT15M**
  （每小时 4 次只读校验），ExecutionTimeLimit=PT10M < 间隔，配合
  MultipleInstancesPolicy=IgnoreNew 防重叠。
- **预算交叉 pin（调度器绝不先于内部超时杀整任务）**：drift watch 单轮
  最坏只读子进程硬顶 = COMPOSE_PS_TIMEOUT_SECONDS 60s + 7 容器 inspect
  各 30s + 2 image inspect 各 30s = **330s**（M14-127 常量）；选择
  PT10M=600s > 330s（执行时限覆盖单轮硬顶并留 ≥270s 收尾余量）、
  PT15M=900s > 600s（间隔大于执行时限，调度器侧不自我重叠）——契约测试
  逐项断言 interval > limit > 330s。
- 归属判定（fail-closed，逐项精确；适配 M14-06 实证的 Task Scheduler
  归一化）：URI 为本工具标识之一（写入值或归一化 ``\\<TASK_NAME>``——该
  形态对任何同名任务都会出现，不具区分性）+ Description 持久归属标记精确
  相等 + Command/Arguments/WorkingDirectory 与全部安全相关设置逐项精确；
...
- secret 纪律：零 secret 产生/落盘/传输；本工具不读任何 env 值，零网络，
  零生产采集——本工具只管理任务注册面，从不运行 production_drift_watch
  本体（那是 VBS→Task Scheduler 侧的 supervisor 获准行为）。
...
run_production_drift_watch_silent.vbs"``；WorkingDirectory=<repo>。
...
elevated token 下成功；本开发回合禁止人工执行 install/uninstall）：
  python tools/ops/production_drift_watch_task.py plan
  python tools/ops/production_drift_watch_task.py generate
  python tools/ops/production_drift_watch_task.py status
  python tools/ops/production_drift_watch_task.py install --confirm "EXECUTE PRODUCTION DRIFT WATCH SCHEDULER CHANGE"
  python tools/ops/production_drift_watch_task.py uninstall --confirm "EXECUTE PRODUCTION DRIFT WATCH SCHEDULER CHANGE"
"""
...
TASK_NAME = "AIOS-Production-Drift-Watch"
TASK_URI = "urn:aios:m14-129:production-drift-watch"
...
TASK_DESCRIPTION = (
    "AIOS production drift watch (M14-129) hidden scheduled read-only "
    "image-anchor verification; managed by tools/ops/production_drift_watch_task.py"
    " - do not edit by hand"
)
#: 保守重复间隔：PT15M=900s（每小时 4 次只读校验）；执行时限 PT10M=600s
#: < 间隔（调度器侧不会自我重叠，另有 IgnoreNew 兜底）。PT10M=600s 同时 >
#: drift watch 单轮最坏只读子进程硬顶之和 60 + 7×30 + 2×30 = 330s
#: （M14-127 常量 COMPOSE_PS_TIMEOUT_SECONDS / INSPECT_TIMEOUT_SECONDS /
#: STACK_SERVICES / EXPECTED_ANCHORS；契约测试交叉 pin——调度器绝不先于
#: 内部超时杀整任务，留 ≥270s 给报告写入与收尾）。
...
TASK_CONFIRM_PHRASE = "EXECUTE PRODUCTION DRIFT WATCH SCHEDULER CHANGE"

#: generate 输出（gitignored，与 drift watch 报告同根的专属子目录）
...
def _repo_paths(repo_root: Path) -> dict[str, Path]:
    return {
        "drift_watch": repo_root / "tools" / "ops" / "production_drift_watch.py",
...
def is_create_command(argv: tuple[str, ...] | list[str]) -> bool:
    ...
    路径前缀而非旗标（Linux CI 的 tempfile 产出 ``/tmp/...`` 路径，M14-14
    PR #89 R3 实证）；旗标形态防护不靠首字符黑名单，而靠**结构**：必须以
    ``.xml`` 结尾、不以 ``-`` 起头、且之后无任何多余 token（``-F``/``/F``/
    ``x.xml/F``/追加旗标/非 .xml 路径/错任务名一律拒绝）。白名单不因该值
    位置放宽到其它任何位置。"""
    tokens = tuple(str(item) for item in argv)
...
def preflight(*, repo_root: Path, log) -> bool:
    """结构预检（只读）：drift watch 工具脚本 + VBS wrapper + repo 自带
    venv python（wrapper 实际调用目标，无覆盖面——feature worktree 无
    .venv 即拒绝）。"""
    paths = _repo_paths(repo_root)
    checks = [
        ("repo 根目录", repo_root.is_dir()),
        ("drift watch 采集脚本", paths["drift_watch"].is_file()),
...
def cmd_plan(*, runner: GatedSchtasks, repo_root: Path, log) -> int:
    ...
    log(f"=== production drift watch 计划任务 PLAN（零写操作）: task={TASK_NAME} ===")
    ...
    log(f"  Action: {WSCRIPT} //B //Nologo \"<repo>\\tools\\ops\\run_production_drift_watch_silent.vbs\"")
    log(f"  WorkingDirectory: <repo> = {repo_root}")
    log("  回滚: python tools/ops/production_drift_watch_task.py uninstall"
        " --confirm \"EXECUTE PRODUCTION DRIFT WATCH SCHEDULER CHANGE\"（仅删本工具精确拥有的任务）")
    log('  实际安装需: install --confirm "EXECUTE PRODUCTION DRIFT WATCH SCHEDULER CHANGE"'
...
def cmd_generate(*, repo_root: Path, log, out_dir: Path | None = None) -> int:
    ...
    **完全一致**）——外部解析器（System.Xml 等）可直接加载；M14-14 R2
    ...
    """
    log(f"=== production drift watch 计划任务 GENERATE（零调度器改动）: task={TASK_NAME} ===")
    directory = out_dir if out_dir is not None else (
        repo_root / ".verify" / "artifacts" / "m14-129-production-drift-watch-task")
...
def cmd_status(*, runner: GatedSchtasks, repo_root: Path, log) -> int:
    ...
    log(f"=== production drift watch 计划任务 STATUS（只读）: task={TASK_NAME} ===")
    query = query_task(runner, repo_root)
    if query.state == STATE_INSTALLED:
...
def cmd_install(*, runner: GatedSchtasks, repo_root: Path, log) -> int:
    ...
    log(f"=== production drift watch 计划任务 INSTALL: task={TASK_NAME} ===")
    ...
    try:
        with tempfile.NamedTemporaryFile("wb", suffix=".xml", prefix="aios-production-drift-watch-",
...
def cmd_uninstall(*, runner: GatedSchtasks, repo_root: Path, log) -> int:
    ...
    """
    log(f"=== production drift watch 计划任务 UNINSTALL: task={TASK_NAME} ===")
    query = query_task(runner, repo_root)
    if query.state == STATE_MISSING:
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="production_drift_watch_task.py",
        description="M14-129 production drift watch 独立周期计划任务 readiness 管理器"
...
def main(argv: list[str] | None = None, *, runner: GatedSchtasks | None = None,
    ...
    if log is None:
        def log(message: str) -> None:
            print(f"[drift-watch-task] {message}", flush=True)
