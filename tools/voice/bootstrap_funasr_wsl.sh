...
#
# 修正轮（supervisor corrections）：不再假设 apt python3.11——优先经 uv 隔离管理
#   （uv venv --python 3.11 --seed；uv 探测含 ~/.local/bin，非登录 bash 的 PATH
#   不含它），回退系统 python3.11 → 系统 python3（版本打印留档）。
#
...
#   FUNASR_UV            uv 可执行文件路径（默认自动探测 command -v / ~/.local/bin/uv）
#   FUNASR_PYTHON        直接指定 python 解释器（跳过 uv；不推荐，破坏隔离性）
...
# ---- venv：uv 隔离管理 Python 3.11（无 apt 假设；--seed 装入 pip）----
if [ ! -x "$VENV_DIR/bin/python" ]; then
  if [ -n "${FUNASR_PYTHON:-}" ]; then
    PY="$FUNASR_PYTHON"
    say "使用显式指定解释器（FUNASR_PYTHON）创建 venv"
    "$PY" -m venv "$VENV_DIR" || fail "venv 创建失败（FUNASR_PYTHON 指定的解释器不可用）"
  else
    UV_BIN=""
    if [ -n "${FUNASR_UV:-}" ]; then
      UV_BIN="$FUNASR_UV"
    ...
    if [ -n "$UV_BIN" ]; then
      say "uv 创建隔离 venv（Python 3.11；本机缺失时由 uv 下载托管解释器，无需 apt/sudo）"
      "$UV_BIN" venv --python 3.11 --seed "$VENV_DIR" || fail "uv venv 失败（网络受限时 uv 下载解释器需代理）"
    elif command -v python3.11 >/dev/null 2>&1; then
      say "未找到 uv，回退系统 python3.11（推荐安装 uv：curl -LsSf https://astral.sh/uv/install.sh | sh）"
      python3.11 -m venv "$VENV_DIR" || fail "venv 创建失败（Debian/Ubuntu 需 python3-venv 包）"
    elif command -v python3 >/dev/null 2>&1; then
      say "未找到 uv 与 python3.11，回退 python3（funasr 支持 3.9+；版本打印留档）"
      python3 -m venv "$VENV_DIR" || fail "venv 创建失败（Debian/Ubuntu 需 python3-venv 包）"
    else
      fail "无 uv 且无系统 python3（安装 uv 后重试：curl -LsSf https://astral.sh/uv/install.sh | sh）"
    fi
  fi
fi
say "venv Python: $("$VENV_DIR/bin/python" --version 2>&1)"

# ---- 依赖（幂等：满足即 no-op；CPU torch 先装，funasr 不 pin torch 不覆盖）----
