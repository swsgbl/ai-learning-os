#!/usr/bin/env python
"""M14-77 语音健康 sidecar 看护计划任务 readiness 管理器（持续保活调度面）。

管理隐藏周期计划任务（Task Scheduler），把既有幂等 ensure 入口
（``tools/voice/voice_health_sidecar_control.py start``）经静默 VBS wrapper
（``run_voice_sidecar_watchdog_silent.vbs``）挂到固定间隔触发。

背景（2026-09-21 生产事实）：FunASR/CosyVoice 引擎均 managed-running、
health 200，但 voice_health_sidecar PID 6660 于 01:00 后静默退出——15 分钟
监控管道（monitoring_pipeline 固定 argv ``--voice-health-source sidecar``）
因 sidecar 端点不可达连续约 7 小时 monitor exit 2，long-soak 被污染。根因
缺陷：sidecar 是「spawn 一次、无看护」的未托管进程；控制器 ``start`` 的
幂等自愈（stale manifest → 清理 → 全新启动，全部生产保护核验保留）已存在，
但生产拓扑中无任何周期性调用者——唯一触发是人工。本任务把「定期 ensure」
固化为计划任务，把 sidecar 死亡暴露窗从 ∞ 收敛到一个看护周期（PT5M），
绝不改变 production_monitor 的只读/fail-closed/不伪造健康语义。

设计纪律（与 tools/ops/monitoring_pipeline_task.py 同源，M14-06 R2–R4
实证模式移植）：
- 纯标准库；平台命令经注入 Runner + **结构性白名单门**（GatedSchtasks）——
  ...
  FakeSchtasks——绝不触碰真实 schtasks 的写路径。
- **install/uninstall 各自需精确确认短语**（``--confirm
  "EXECUTE VOICE SIDECAR WATCHDOG SCHEDULER CHANGE"`` 一字不差），缺一即
...
- 绝不覆盖同名任务：install 前必先只读 query + 二次全量列表复核，已存在
  ...
  查询失败绝不 force、绝不删除。
- 固定任务身份 + 保守重复间隔：``AIOS-Voice-Sidecar-Watchdog`` /
  ``urn:aios:m14-77:voice-sidecar-watchdog``；TimeTrigger 重复间隔 **PT5M**
  （死亡暴露窗收敛到一个看护周期，且 < 监控管道 PT15M——15 分钟管道最多
  污染一轮）；ExecutionTimeLimit=PT4M < 间隔，配合
  MultipleInstancesPolicy=IgnoreNew 与控制器自身 ControlLock 双重防重叠。
- 单轮 ensure 预算（WATCHDOG_RUN_BUDGET_SECONDS=90s）：控制器 ``start``
  最坏推算 ≈ 前置归属 probe（PROBE_TIMEOUT 15s）+ spawn 后 status 落档
  等待（START_STATUS_TIMEOUT 20s）+ 身份 probe（PROBE_TIMEOUT 15s）+
  幂等分支双端口健康探测（HEALTH_TIMEOUT 5s ×2）≈ 70s，加解释器/wsl.exe
  启动余量；PT4M=240s 预算充裕覆盖（测试交叉 pin）。
- 归属判定（fail-closed，逐项精确；适配 M14-06 实证的 Task Scheduler 归一
  化）：URI 为本工具标识之一（写入值或归一化 ``\\<TASK_NAME>``）+
  Description 持久归属标记精确相等 + Command/Arguments/WorkingDirectory
  与全部安全相关设置逐项精确；归一化省略的默认值元素仅在父节点在场且其余
  字段全部精确时按 Windows 默认值认可。**诚实边界**：TimeTrigger/
  Repetition/StartBoundary 的注册后归一化行为以 M14-06（LogonTrigger 形态）
  与 M14-14（TimeTrigger 形态）实证先例为准——首次 supervisor 注册若暴露
  归一化漂移，按 malformed fail-closed（绝不弱解放行）。
- /XML 解码：schtasks /XML 输出编码随捕获通道而变——一律经 ``run_raw``
  取原始字节 + ``decode_schtasks_xml`` 按字节形态严格解码四形态；之外/
...
Hidden=true；TimeTrigger(Enabled)+Repetition Interval=PT5M（无 Duration=
...
ExecutionTimeLimit=PT4M；电池不禁启不停；Action=wscript.exe，
Arguments=``//B //Nologo "<repo>\\tools\\voice\\
run_voice_sidecar_watchdog_silent.vbs"``；WorkingDirectory=<repo>。

运维语义（诚实边界）：
- 看护任务在线期间，sidecar 期望恒运行——人工维护需先停看护任务
  （本工具 uninstall 或 schtasks /Change /Disable，均 supervisor 操作），
  否则 stop 后 ≤5 分钟内会被幂等 ensure 重新拉起（这是设计意图，非缺陷）。
- ``start`` 的退出码经 VBS 原样透传给 Task Scheduler（0=健康/已重启/幂等；
  1=启动失败；3=安全拒绝/WSL 管理不可用）——非零「上次运行结果」可见不
  遮蔽，下一轮自动重试。绝不以任何方式把失败伪装为成功。
...
用法（仓库根；install 需提升令牌——M14-06/M14-14 生产实证 schtasks
/Create 仅在 elevated token 下成功；本开发回合禁止人工执行
install/uninstall）：
  python tools/voice/voice_sidecar_watchdog_task.py plan
  python tools/voice/voice_sidecar_watchdog_task.py generate
  python tools/voice/voice_sidecar_watchdog_task.py status
  python tools/voice/voice_sidecar_watchdog_task.py install --confirm "EXECUTE VOICE SIDECAR WATCHDOG SCHEDULER CHANGE"
  python tools/voice/voice_sidecar_watchdog_task.py uninstall --confirm "EXECUTE VOICE SIDECAR WATCHDOG SCHEDULER CHANGE"
"""
...
TASK_NAME = "AIOS-Voice-Sidecar-Watchdog"
TASK_URI = "urn:aios:m14-77:voice-sidecar-watchdog"
...
TASK_DESCRIPTION = (
    "AIOS voice health sidecar watchdog (M14-77) hidden scheduled idempotent"
    " ensure; managed by tools/voice/voice_sidecar_watchdog_task.py - do not"
    " edit by hand"
)
#: 保守重复间隔：PT5M——sidecar 死亡暴露窗收敛到一个看护周期，且 < 监控
#: 管道 PT15M（15 分钟管道最多污染一轮）；执行时限 PT4M < 间隔（调度器侧
#: 不会自我重叠，另有 IgnoreNew + 控制器 ControlLock 兜底）。
REPETITION_INTERVAL = "PT5M"
EXECUTION_TIME_LIMIT = "PT4M"
#: 单轮 ensure（controller start）最坏预算上界（秒）：probe×2 + status 落档
#: 等待 + 幂等分支双端口健康探测 + 解释器/wsl.exe 启动余量（推导见模块
#: docstring；测试与 voice_health_sidecar_control 常量交叉 pin）。
WATCHDOG_RUN_BUDGET_SECONDS = 90.0
#: 固定 StartBoundary（过去时刻）：注册即生效、按间隔无限期重复
#: （Repetition 不带 Duration = indefinitely；M14-14 同款形态）。
START_BOUNDARY = "2026-01-01T00:00:00"
WSCRIPT = "wscript.exe"
WSCRIPT_ARGS_TEMPLATE = '//B //Nologo "{vbs}"'

#: install/uninstall 各自的精确确认短语（一字不差；本开发回合永不满足执行）
TASK_CONFIRM_PHRASE = "EXECUTE VOICE SIDECAR WATCHDOG SCHEDULER CHANGE"

#: generate 输出（gitignored，与监控任务管理器同款纪律）
...
def _repo_paths(repo_root: Path) -> dict[str, Path]:
    return {
        "controller": repo_root / "tools" / "voice" / "voice_health_sidecar_control.py",
        "sidecar": repo_root / "tools" / "voice" / "voice_health_sidecar.py",
...
def is_create_command(argv: tuple[str, ...] | list[str]) -> bool:
    ...
    路径前缀而非旗标（M14-14 PR #89 CI 回归：Linux CI 的 tempfile 产出
    ``/tmp/.../aios-*.xml``，首字符黑名单会误拒）；旗标形态防护不靠首字符
    黑名单，而靠**结构**：必须以 ``.xml`` 结尾、不以 ``-`` 起头、且之后无
    任何多余 token（``-F``/``/F``/``x.xml/F``/追加旗标/非 .xml 路径/错任务
    名一律拒绝）。白名单不因该值位置放宽到其它任何位置。
    """
    tokens = tuple(str(item) for item in argv)
...
def preflight(*, repo_root: Path, log) -> bool:
    """结构预检（只读）：控制器脚本（幂等 ensure 入口）+ sidecar 脚本 +
    VBS wrapper + repo 自带 venv python（wrapper 实际调用目标，无覆盖面——
    feature worktree 无 .venv 即拒绝）。"""
    paths = _repo_paths(repo_root)
    checks = [
        ("repo 根目录", repo_root.is_dir()),
        ("sidecar 控制器脚本（幂等 ensure 入口）", paths["controller"].is_file()),
        ("sidecar 脚本（受控目标）", paths["sidecar"].is_file()),
...
def cmd_plan(*, runner: GatedSchtasks, repo_root: Path, log) -> int:
    ...
    log(f"=== 语音 sidecar 看护计划任务 PLAN（零写操作）: task={TASK_NAME} ===")
    ...
    log(f"  Action: {WSCRIPT} //B //Nologo \"<repo>\\tools\\voice\\run_voice_sidecar_watchdog_silent.vbs\"")
    log(f"  单轮 ensure: voice_health_sidecar_control.py start（幂等；预算 {WATCHDOG_RUN_BUDGET_SECONDS:.0f}s < {EXECUTION_TIME_LIMIT}）")
    log(f"  WorkingDirectory: <repo> = {repo_root}")
    log("  回滚: python tools/voice/voice_sidecar_watchdog_task.py uninstall"
        " --confirm \"EXECUTE VOICE SIDECAR WATCHDOG SCHEDULER CHANGE\"（仅删本工具精确拥有的任务）")
    log('  实际安装需: install --confirm "EXECUTE VOICE SIDECAR WATCHDOG SCHEDULER CHANGE"'
...
def cmd_generate(*, repo_root: Path, log, out_dir: Path | None = None) -> int:
    ...
    **完全一致**）——外部解析器（System.Xml 等）可直接加载。落盘后**回读
    ...
    """
    log(f"=== 语音 sidecar 看护计划任务 GENERATE（零调度器改动）: task={TASK_NAME} ===")
    directory = out_dir if out_dir is not None else (
        repo_root / ".verify" / "artifacts" / "m14-77-voice-sidecar-watchdog")
...
def cmd_status(*, runner: GatedSchtasks, repo_root: Path, log) -> int:
    ...
    log(f"=== 语音 sidecar 看护计划任务 STATUS（只读）: task={TASK_NAME} ===")
    query = query_task(runner, repo_root)
    if query.state == STATE_INSTALLED:
...
def cmd_install(*, runner: GatedSchtasks, repo_root: Path, log) -> int:
    ...
    FakeSchtasks）。预检+无同名+临时 XML+归一化容错复查全过才 /Create。"""
    log(f"=== 语音 sidecar 看护计划任务 INSTALL: task={TASK_NAME} ===")
    ...
    try:
        with tempfile.NamedTemporaryFile("wb", suffix=".xml", prefix="aios-voice-sidecar-watchdog-",
...
def cmd_uninstall(*, runner: GatedSchtasks, repo_root: Path, log) -> int:
    ...
    """
    log(f"=== 语音 sidecar 看护计划任务 UNINSTALL: task={TASK_NAME} ===")
    query = query_task(runner, repo_root)
    if query.state == STATE_MISSING:
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="voice_sidecar_watchdog_task.py",
        description="M14-77 语音健康 sidecar 看护计划任务 readiness 管理器"
...
def main(argv: list[str] | None = None, *, runner: GatedSchtasks | None = None,
    ...
    if log is None:
        def log(message: str) -> None:
            print(f"[sidecar-watchdog-task] {message}", flush=True)
